...
class CriticNeuralODE(SACPolicy):
    
    def make_features_extractor_actor(self) -> BaseFeaturesExtractor:
        """Helper method to create a features extractor."""
        obs_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(6,), dtype=np.float32)

        return self.features_extractor_class(obs_space, **self.features_extractor_kwargs)

    def _update_features_extractor_actor(
        self,
        ...
        if features_extractor is None:
            ...
            features_extractor = self.make_features_extractor_actor()
        net_kwargs.update(dict(features_extractor=features_extractor, features_dim=features_extractor.features_dim))
        return net_kwargs
    
    def make_actor(self, features_extractor: Optional[BaseFeaturesExtractor] = None) -> Actor:
        # customize so that the actor has a different shaped input
        
        # actor_kwargs = self.actor_kwargs
        
        # # change actor kwargs to use a different observation space. Input is now joint positions, and joint actions
        # obs_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(6,), dtype=np.float32)
        # actor_kwargs['observation_space'] = obs_space
        
        # actor_kwargs = self._update_features_extractor_actor(actor_kwargs, features_extractor)
        
        
        # return Actor(**actor_kwargs).to(self.device)
        
        # just make a fucking mlp
        ...
        return SimpleActor().to(self.device)
    ...
    def _predict_lbfgs(self, observation: PyTorchObs, deterministic: bool = False) -> th.Tensor: 
        ...
        #     # q_value = th.min(q_values[0], q_values[1])
        #     q_value = q_values[0]
        ...
        def closure():
            ...
            with th.no_grad():
                if action_tensor.grad is not None:
                    action_tensor.grad.add_(grads)
            
            return loss
        
        # initial losses
        # initial_loss = closure_losses().item()
        
        # run it
        optimizer.step(closure) #type:ignore
        
        # final losses
        # final_loss = closure_losses().item()
...
class CriticNeuralODEAlgorithm(SAC):
    """
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        self.train_critic(gradient_steps, batch_size)
        
        self.train_actor(gradient_steps, batch_size)
    ...
    def train_actor(self, gradient_steps: int, batch_size: int = 64) -> None:
        """
        neural ode. target gradient from the critic. predicted gradient as the actor output.
        """            
        ...
        for gradient_step in range(gradient_steps):
            ...
            # make a grad-full action for the critic
            actions = replay_data.actions.clone().detach().requires_grad_(True)
                
            # forward critic
            current_q_values = self.critic(replay_data.observations, actions)
            
            # min critic value across ensemble
            min_current_q_values, _ = th.min(th.cat(current_q_values, dim=1), dim=1, keepdim=True)
            
            # we want to minimize the -qval
            loss = -min_current_q_values.mean()
            
            # backward
            loss.backward()
            
            # get the target grads from the critic
            assert(actions.grad is not None)
            target_action_grads = actions.grad.detach()
            
            # treat the actions like another observation
            assert(isinstance(replay_data.observations, th.Tensor))
            obs = th.cat([replay_data.observations, actions.detach()], dim=1)
            
            # now do the forward pass on the actor
            predicted_action_grads = self.actor(obs)
            
            # mse actor loss
            actor_loss = F.mse_loss(predicted_action_grads, target_action_grads)
            
            assert isinstance(actor_loss, th.Tensor)  # for type checker
            actor_losses.append(actor_loss.item())  # type: ignore[union-attr]
    ...
    def train_critic(self, gradient_steps: int, batch_size: int = 64) -> None:
        """
